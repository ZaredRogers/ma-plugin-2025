	...
	const post = useSelect(
		(select) => {
			...
			return select('core').getEntityRecord(
				'postType',
				context.postType || 'medical-academic-enhancements',
				postId
			);
		},
		...
		<>
			<InspectorControls>
				<PanelBody title={__('Display Settings', 'medical-academic-enhancements')}>
					...
					/>
					<ToggleControl
						label={__('Link to Post', 'medical-academic-enhancements')}
						checked={linkToPost}
						onChange={(value) =>
			...
			<div {...blockProps}>
				...
				<div className="wp-block-example_plugin-example-plugin-card__content">
					{displayTitle && post && (
						<h3 className="wp-block-example_plugin-example-plugin-card__title">
							{post.title?.rendered ||
								__('Untitled', 'medical-academic-enhancements')}
						</h3>
					)}

					{displaySubtitle && (
						<p className="wp-block-example_plugin-example-plugin-card__subtitle">
							{__('Subtitle placeholder', 'medical-academic-enhancements')}
						</p>
					)}
				...
				{!post && (
					<p className="wp-block-example_plugin-example-plugin-card__placeholder">
						{__('Select a post to display.', 'medical-academic-enhancements')}
					</p>
				)}
			</div>
		</>
